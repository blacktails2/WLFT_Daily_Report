...
export default function ReportPage() {
  ...
  data?.totals.forEach((t) => { taskColors[t.taskName] = t.taskColor; });
          ...
          <div className="flex items-center gap-1 mr-2">
            {(["week", "month", "year"] as Period[]).map((p) => (
              <button
                ...
                className={`px-3 py-1.5 text-sm rounded-full transition-all ease-in duration-100 capitalize ${
                  period === p
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data.daily} barCategoryGap="30%">
                  ...
                  <Tooltip
                    ...
                    wrapperStyle={{ transition: "opacity 150ms ease-out" }}
                  />
                  {taskNames.map((name) => (
                    <Bar
                      key={name}
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-2 justify-center -mt-8">
              {data.totals.map((t) => (
                <div key={t.taskId} className="flex items-center gap-2">
                  ...
                  <span className="text-sm">{t.taskName} ({t.totalHours}h)</span>
                </div>
              ))}
            </div>
          </div>
